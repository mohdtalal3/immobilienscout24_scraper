...
from immo24_scraper import run_scraper_for_account
...
def get_accounts_ready_to_scrape(supabase: Client):
    ...
    """
    try:
        # Fetch all immoscout24 accounts
        response = supabase.table('accounts').select('*').eq('website', 'immoscout24').execute()
...
@app.route('/accounts')
def accounts():
    """Get all immoscout24 accounts from Supabase."""
    try:
        response = supabase.table('accounts').select('id, email, website, last_updated_at, configuration').eq('website', 'immoscout24').execute()
...
if __name__ == '__main__':
    ...
    logger.info(f"\n{'='*60}")
    logger.info("🚀 IMMOSCOUT24 SCRAPER BACKEND STARTED")
    logger.info(f"{'='*60}")
    logger.info(f"🌐 Flask server starting on http://0.0.0.0:5002")
    ...
    logger.info(f"{'='*60}")
    logger.info(f"📥 Quick Download: http://localhost:5002/logs/download")
    logger.info(f"{'='*60}\n")
